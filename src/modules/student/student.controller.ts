...
import { updateStudentProfile, getStudentDetails, getMyStudentProfile } from "./student.service.js";
...
export async function getStudentDetailsController(req: FastifyRequest, reply: FastifyReply) {
  const { id } = req.params as { id: string };
  try {
    const data = await getStudentDetails(id);
    return reply.send({ success: true, data });
  } catch (err: any) {
    if (err.message === "NOT_FOUND") return reply.status(404).send({ success: false, message: "Student not found" });
    throw err;
  }
}

export async function getMyStudentProfileController(req: FastifyRequest, reply: FastifyReply) {
  try {
    const data = await getMyStudentProfile(req.user!.userId);
    return reply.send({ success: true, data });
  } catch (err: any) {
    if (err.message === "NOT_FOUND") return reply.status(404).send({ success: false, message: "Student not found" });
    throw err;
